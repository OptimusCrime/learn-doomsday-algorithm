...
export const AnchorDays = () => {
  ...
  const buttons = [2, 3, 5, 0];

  return (
    <div>
      <div>
        <p className="font-bold">
          What was the anchor day between {FROM_YEAR} - {TO_YEAR}?
        </p>
      </div>
        ...
        {buttons.map((value) => (
          <button
            className={`border p-2 rounded ${
              state.showAnswer && value === state.guessed
                ? state.guessed === state.currentCentury.anchorDay
                  ? 'bg-green-400'
                  : 'bg-red-400'
                : ''
            }`}
